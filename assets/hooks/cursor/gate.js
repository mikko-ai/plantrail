...
function invalidateOnPlanChange(status) {
...
function plantrailHome() {
  return join(homedir(), ".plantrail");
}
function plantrailKeyPath() {
  return join(plantrailHome(), "key");
}
function authorityApprovalPath(runId) {
...
import { chmodSync, existsSync as existsSync2, mkdirSync as mkdirSync2, readFileSync as readFileSync2, writeFileSync as writeFileSync2 } from "node:fs";
function sha256(content) {
  return createHash("sha256").update(content, "utf8").digest("hex");
}
function ensurePlantrailKey() {
  const keyPath = plantrailKeyPath();
  mkdirSync2(plantrailHome(), { recursive: true, mode: 448 });
  if (!existsSync2(keyPath)) {
    const key = randomBytes(32);
    writeFileSync2(keyPath, key);
    chmodSync(keyPath, 384);
    return key;
  }
  chmodSync(keyPath, 384);
  return readFileSync2(keyPath);
}
function signPlanHash(planHash) {
  const key = ensurePlantrailKey();
  return createHmac("sha256", key).update(planHash, "utf8").digest("hex");
}
function verifyPlanSignature(planHash, signature) {
...
function verifyPlanIntegrity(planContent, planHash, signature) {
...
function findMatchingStep(actionType, target, steps) {
...
function isRunArtifactPath(projectRoot, runId, target) {
  const rel = relativeTarget(projectRoot, target);
  return rel.includes(`.agent-loop/runs/${runId}/plan.md`) || rel.includes(`.agent-loop/runs/${runId}/review.md`);
}
function evaluateGate(input, approval) {
  ...
  if (isPreApproval(status)) {
    ...
    if (actionType === "write_file" && isRunArtifactPath(project_root, run_id, target)) {
...
function runGate(projectRoot, input) {
  ...
  if (result.decision === "allow" && approval.status === "approved") {
    updateApproval(
      projectRoot,
      runId,
      (record) => record.status === "approved" ? { ...record, status: "doing" } : record
    );
  }
...
function cursorStopResponse(result) {
...
function normalize(raw) {
  ...
  const tool = event.includes("Shell") ? "beforeShellExecution" : String(raw.tool_name ?? raw.tool ?? event);
